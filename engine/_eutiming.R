...
gov <- as.data.frame(read_dta(file.path(fw,"datasets/../aspm_lr_quarterly.dta")))  # falls vorhanden
...
# GOV_POS vorberechnen (wie es die App mitliefern wuerde)
...
t0<-Sys.time(); govpos <- estimate_aspm(d, specs, ideo="ja10f", referenda=ref, verbose=FALSE)$quarterly
cat("estimate_aspm (GOV_POS):", round(as.numeric(difftime(Sys.time(),t0,units="secs")),1),"s\n")
# reine EU-Schaetzung (das laeuft bei Custom in WebR)
t1<-Sys.time(); out <- estimate_eu(d, "ja10f", govpos, external=ext); dt<-as.numeric(difftime(Sys.time(),t1,units="secs"))
cat("estimate_eu (nur EU):", round(dt,1),"s ->  WebR grob x3-8\n")
cat("EU-Datensatz-Groesse: Mitglieder-Zeilen =", sum(!is.na(d$p601)), " Spalten fuer EU noetig?\n")
